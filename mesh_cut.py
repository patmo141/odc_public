...
def list_shift(seq, n):
    '''
    shift a list by n elements.  The n'th element becomes th 0th element
    seq = [a,b,c,d,e,f,g,h]
    ...
    list_shift(list(reversed(seq)), 2)
    > [f,e,d,c,b,a,h,g]
    
    list(reversed(list_shift(seq, 2)))
    > [b,a,h,g,f,e,d,c]
    '''
    n = n % len(seq)
    return seq[n:] + seq[:n]
...
def cross_section_walker(bme, pt, no, f_ind_from, e_ind_from, co_from, epsilon, max_steps = 100000):
    '''
    bme -  bmesh
    ...
    iters = 0
    while True and iters <= max_steps:
        iters += 1
        # find farthest point
        cross_eds = find_sorted_bmedges_crossing_plane(pt, no, f_cur.edges, epsilon, e_ind_from, co_from)
        edge, i = cross_eds[0]
        
        #edge,i = find_distant_bmedge_crossing_plane(pt, no, f_cur.edges, epsilon, e_ind_from, co_from)
        verts += [i]
        ...
        if find_next in f_inds_dict:   #we might have looped, ngons can be crossed multiple times
            ...
            if len(bme.faces[find_next].edges) <= 4: #can only cross a quad or tri onces
                print('quad or tri, we have looped')
                looped = True
...
def cross_section_2seeds_ver1(bme, mx, point, normal, seed_index0, co_0, seed_index1, co_1, max_tests = 10000, debug = True, prev_face = None, epsilon = .0000001, topo_limit = True):
    '''
    '''
...
def cross_section_seed_ver1(bme, mx, 
    ...
    # start walking one way around bmesh
    verts0,looped = cross_section_walker(bme, pt, no, seed_index, ei0_max[0].index, ei0_max[1], epsilon, max_steps = max_tests)
    ...
    verts1,looped = cross_section_walker(bme, pt, no, seed_index, ei1_max[0].index, ei1_max[1], epsilon, max_steps = max_tests)
