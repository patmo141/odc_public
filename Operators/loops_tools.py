...
original author": "Bart Crouch"
"wiki_url": "http://wiki.blender.org/index.php/Extensions:2.6/Py/"
adapted and modified for use use in Open Dental CAD
by Patrick Moore
...
def get_connected_input(object, bm, scene, mode):
...
def get_derived_bmesh(obj, bm, scene):
    ...
    if 'MIRROR' in [mod.type for mod in obj.modifiers if mod.show_viewport]:
        ...
        # get derived mesh
        bm_mod = bmesh.new()
        mesh_mod = obj.to_mesh(scene, True, 'PREVIEW')
...
def relax_loops_util(bme, selected_edges, iterations, influence = .5):
    ...
    for iteration in range(iterations):
        # calculate splines and new positions
        tknots, tpoints = relax_calculate_t(bme, knots, points, False)
        splines = []
        for i in range(len(knots)):
            splines.append(calculate_splines('cubic', bme,
                tknots[i], knots[i]))
        move = [relax_calculate_verts(bme, 'cubic',
            tknots, knots, tpoints, points, splines)]
        
        for loop in move:
            for index, loc in loop:
                new_loc = loc*(influence) + \
                    bme.verts[index].co*((1-influence))
                bme.verts[index].co = new_loc
    bme.normal_update()
