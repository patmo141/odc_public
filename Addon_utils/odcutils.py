...
def get_settings():
    if not get_settings.cached_settings:
        ...
        get_settings.cached_settings = addons[addon_name].preferences
...
def fill_bmesh_loop_scale(bme, bmedges, res, debug = False):
    ...
    for i in range(1,step):  #step
        ...
        if len(prev_loop) == 0 and len(verts_extrude) != 0:
            print('this is the scenario where last remove doubles messes something up')
            for v in bme.verts:
...
def simplypoly(splineVerts, order): #options
    ...
    # get distance values per vert - same as Ramer-Douglas-Peucker
    # but for every vert
    '''
...
def vert_group_inds_get(context, ob, vgroup, debug = False):
    ...
    if you need it to not disrupt your scene, use scene preservation and reconstruct
    '''
    if debug:
        start = time.time()
...
def iterate(points, newVerts, error):
    ...
    error - distance above/below chord which makes vert considered a feature
    
    return:
    new -  list of vertex indicies (mapping to arg points) representing identified feature points
    or
...
def  tracer_settle(tracer, settling_treshold = .1, epsilon = .2, max_settles=10, debug = False, guess_on_flat = True):
    '''
    trace is a blender mesh object, generated by generate_trace, shrinkwrapped to another object
    ...
    while n_settles < max_settles:
        ...
        for j, loop in enumerate(spokes):
            ...
            #note the transform to world coords....ob.to_mesh() gives deformed mesh 
        ...
        if not do_plane_re_orient and debug:
            print("didnt reorient tracer because dot product was %f" % norm.dot(tracer.rotation_quaternion * Vector((0,0,1))))
...
def center_objects(context, obs, set_origins = False, bbox = True):
    ...
    #Should this handle everything well for parenting?
    for ob in obs:
        for i in range(0,3):
